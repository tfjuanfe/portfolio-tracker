...
class ModifyAsset(QWidget):
    ...
    def setup_ui(self):
        ...
        # ── Select Asset ──
        select_label = QLabel("Select Asset to modify:")
        ...
        val_label.setFont(QFont("Arial", 11, QFont.Bold))
        val_label.setStyleSheet("color: #1A1A2E; background: transparent;")
        ...
        left.addWidget(val_label)
        left.addWidget(self.value_input)
    ...
    def handle_save(self):
        if not self.assets:
            QMessageBox.warning(self, "Error", "No assets to modify")
            return
        ...
        if not new_name and not new_value:
            QMessageBox.warning(self, "Error", "Please fill in at least one field to update")
            return

        asset_id      = self.asset_combo.currentData()
        current_index = self.asset_combo.currentIndex()
        current_asset = self.assets[current_index]

        final_name  = new_name  if new_name  else current_asset[2]
        final_value = new_value if new_value else current_asset[4]
        ...
        self.db.modify_asset(asset_id, final_name, final_value)
